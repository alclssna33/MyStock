...
6.  **탭 스타일 (CSS 13번)**: `.stTabs [data-baseweb="tab"]` 기반으로 활성 탭은 보라색 그라데이션, 비활성 탭은 반투명으로 구분됩니다.
7.  **Metric 델타 색상 보존 (CSS 14번)**: `div { color: #FFFFFF }` 규칙에 의해 상승/하락 delta 색상이 덮이는 문제를 방지합니다.
8.  **Expander 스타일 통합 (CSS 16번)**: 레거시 `.streamlit-expanderHeader` 대신 `[data-testid="stExpander"]` 기반으로 다크 인디고 스타일이 적용됩니다.
9.  **전체 현황판 카드 그리드**: `st.dataframe` (엑셀 형태) 대신 `portfolio-card-grid` CSS 클래스를 사용한 반응형 카드 그리드로 대체됐습니다.
10. **진행률 막대그래프**: 각 카드 하단에 진행률(%) 비례 막대그래프가 표시됩니다. 0~49%: 황색, 50~79%: 보라색, 80~100%: 초록색 gradient + glow 효과.
